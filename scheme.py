...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        exp_operator = scheme_eval(first, env)

        # if is a macro procedure, apply macro on the operands
        if isinstance(exp_operator, MacroProcedure):
            return scheme_eval(exp_operator.apply_macro(rest, env), env)
        
        # evaluate each operand
        exp_operand = rest.map(lambda x: scheme_eval(x, env))
        # apply procedure on operands
        return scheme_apply(exp_operator, exp_operand, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    third_arg = None
    while sub_exp != nil:
        # if tail context, pass true argument
        if sub_exp.rest == nil:
            third_arg = True
        result = scheme_eval(sub_exp.first, env, third_arg)
        sub_exp = sub_exp.rest
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        # if found in current frame, return value
        if symbol in self.bindings:
            return self.bindings[symbol]
        parent_frame = self.parent
        # check symbol in parent frame 
        while parent_frame is not None:
            parent_binding = parent_frame.bindings
            if symbol in parent_binding:
                # return value if found in parent frame
                return parent_binding[symbol]
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        current = args
        while current != nil:
            arguments_list.append(current.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        # if its a procedure, bind it to its name
        if isinstance(target, list):
            target = target.first
            env.define(target, target.rest.first)
        else:
            value = expressions.rest.first
            # if target is bound to a expression, get its value
        ...
        # BEGIN PROBLEM 9
        name = target.first
        # formals = target.rest and body = expressions.rest
        exp = Pair(target.rest, expressions.rest)
        # bind the name to the LambdaProcedure instance 
        env.define(name, do_lambda_form(exp, env))
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions != nil:
        rest = expressions
        third_arg, result = None, None
        #eval each sub_expression while is not False and rest != nil
        while result is not False and rest != nil:
            if rest.rest == nil:
                third_arg = True
            result = scheme_eval(rest.first, env, third_arg)
            rest = rest.rest
        return result
    # if empty expressions, return True
    return True
...
def do_or_form(expressions, env):
    ...
    if expressions != nil:
        rest = expressions
        third_arg, result = None, False
        #eval each sub_expression while is not False and rest != nil
        while result is False and rest != nil:
            if rest.rest == nil:
                third_arg = True
            result = scheme_eval(rest.first, env, third_arg)
            rest = rest.rest
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            # eval all sub-expression
            result = eval_all(clause.rest, env)
            # if result of sub-expressions is not None, return result
...
def make_let_frame(bindings, env):
    ...
    first, rest = None, bindings
    # check if the structire of each binding is correct
    while rest is not nil:
        first = rest.first
        validate_form(first, 2, 2)
        rest = rest.rest

    # get the names and values of the bindings
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        name = target.first
        # bind the name to the MacroProcedure instance 
        env.define(name, MacroProcedure(target.rest,expressions.rest, env))
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 15
    def make_call_frame(self, args, env):
        """The parent of the new call frame is the environment in which the call expression is evaluated"""
